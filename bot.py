# bot.py - v13.0 - O Sniper de Arbitragem (Estratégia de Snapshot)
...
def send_welcome(message):
    bot.reply_to(message, "Bot v13.0 (Sniper de Arbitragem) online. Use /status.")
...
class ArbitrageEngine:
    ...
    # =================================================================
    # FUNÇÃO DE SIMULAÇÃO OTIMIZADA (v13.0) - Usando Tickers
    # =================================================================
    def _simular_trade(self, cycle_path, volume_inicial):
        ...
        for i in range(len(cycle_path) - 1):
            ...
            if coin_from != current_coin: return None # Sanity check

            pair_id, side = self._get_pair_details(coin_from, coin_to)
            if not pair_id: return None
            
            ticker = self.tickers.get(pair_id)
            if not ticker: return None # Ticker não encontrado no snapshot

            if side == \'buy\':
                price = ticker.get(\'ask\') # Preço de venda (ask) para comprar
            else: # side == \'sell\'
                price = ticker.get(\'bid\') # Preço de compra (bid) para vender
            
            if not price: return None # Preço não disponível no ticker
            price = Decimal(str(price))

            if side == \'buy\':
                # Estamos gastando \'coin_from\' (ex: USDT) para obter \'coin_to\' (ex: BTC)
                volume_obtido = current_amount / price
            else: # side == \'sell\'
                # Estamos gastando \'coin_from\' (ex: BTC) para obter \'coin_to\' (ex: USDT)
                volume_obtido = current_amount * price
            
            # Aplica a taxa de taker
    ...
    def main_loop(self):
        ...
        while True:
            try:
                # --- OTIMIZAÇÃO: Snapshot de Tickers no início do ciclo ---
...
if __name__ == "__main__":
    logging.info("Iniciando o bot v13.0 (Sniper de Arbitragem)...")
    ...
    bot.send_message(CHAT_ID, "✅ **Bot Gênesis v13.0 (Sniper de Arbitragem) iniciado com sucesso!**")
