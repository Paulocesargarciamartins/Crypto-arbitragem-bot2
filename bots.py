...
# --- Monitoramento de Erros e Exchanges Ativas ---
connection_errors = {}
...
async def get_trade_amount(exchange_name, symbol, is_triangular):
    ...
    is_percentage = (
        triangular_trade_amount_is_percentage
        if is_triangular
        else futures_trade_amount_is_percentage
    )
...
async def simulate_triangular_cycle(cycle, tickers):
    ...
    for instId, action in cycle:
        ...
        if not ticker:
            raise RuntimeError(f"Ticker para {instId} não encontrado.")
    ...
    while True:
        ...
        try:
            ...
            for cycle in dynamic_cycles:
                try:
                    profit_est_pct, profit_est_abs = await simulate_triangular_cycle(
                        cycle, all_tickers
                    )
                    if profit_est_pct > triangular_min_profit_threshold:
                        ...
                        saldo_texto = (
                            f"`{saldo_atual_usdt:.2f} USDT`"
                            if saldo_atual_usdt is not None
                            else "`Não foi possível obter`"
                        )
                        msg = (
                            f"✅ *Arbitragem Triangular (Finalizada)*\n\n"
                            f"`{pares_fmt}`\n"
                            f"Lucro Real: `{profit_est_pct:.3%}` (~`{profit_est_abs:.4f} USDT`)\n"
                            f"Saldo OKX: {saldo_texto}"
                        )
                        registrar_ciclo_triangular(
                            pares_fmt, float(profit_est_pct), float(profit_est_abs), "LIVE", "OK"
                        )
...
async def initialize_futures_exchanges():
    ...
    if not ccxt:
        return
    print("Inicializando exchanges para Futuros...")
...
async def find_futures_opportunities():
    ...
    tasks = {
        name: ex.fetch_tickers(FUTURES_TARGET_PAIRS) for name, ex in active_futures_exchanges.items()
    }
    ...
    for (name, _), res in zip(tasks.items(), results):
        if isinstance(res, Exception):
            ...
            if name not in connection_errors or connection_errors[name] != error_msg:
                ...
                print(f"Erro de conexão com {name} persiste.")
            continue
        if name in connection_errors:
        ...
        for symbol, ticker in res.items():
            ...
            if ticker.get("bid") and ticker.get("ask"):
                prices_by_symbol[symbol].append(
                    {"exchange": name, "bid": Decimal(ticker["bid"]), "ask": Decimal(ticker["ask"])}
                )
    opportunities = []
...
async def loop_bot_futures():
    ...
    await send_telegram_message(
        f"✅ *Bot de Futuros iniciado.* Exchanges: `{', '.join(active_futures_exchanges.keys())}`"
    )
    while True:
...
async def setvolume_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        def parse_volume_arg(arg_str):
            return (Decimal(arg_str[:-1]), True) if arg_str.endswith("%") else (Decimal(arg_str), False)

        (
            triangular_trade_amount,
            triangular_trade_amount_is_percentage,
        ) = parse_volume_arg(args[0])
        futures_trade_amount, futures_trade_amount_is_percentage = parse_volume_arg(args[1])
        tri_text = (
            f"`{triangular_trade_amount}%` do saldo"
            if triangular_trade_amount_is_percentage
            else f"`{triangular_trade_amount}` USDT"
        )
        fut_text = (
            f"`{futures_trade_amount}%` do saldo"
            if futures_trade_amount_is_percentage
            else f"`{futures_trade_amount}` USDT"
        )
...
if __name__ == "__main__":
    ...
    try:
        ...
        if limit < 0:
            await update.message.reply_text("O limite deve ser um número positivo ou zero.")
            return
        futures_trade_limit = limit
        futures_trades_executed = 0
        await update.message.reply_text(
            f"Limite de trades para futuros definido para: {'Ilimitado' if limit == 0 else limit}. Contador resetado."
        )
    except:
